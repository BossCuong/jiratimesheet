...
class report_employee_project(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id = None):
        ...
        if context.get('print_mode') is None :
            if line_id == None :
                results = self.get_all_employee(date_from, date_to, selected_employee, selected_project, line_id)
                total = 0
                for line in results:
            ...
            if line_id :
                results = self.get_all_employee(date_from, date_to, selected_employee, selected_project, line_id[2:])
                ...
                results_task = self.get_all_project(date_from, date_to, selected_employee, selected_project, line_id[2:])
            ...
            results = self.get_all_employee(date_from, date_to, selected_employee, selected_project, None)
            ...
            for line in results:
                ...
                results_task = self.get_all_project(date_from, date_to, selected_employee, selected_project, str(line.get('id')))
    ...
    def get_all_project(self, date_from, date_to, selected_employee, selected_project,line_id = None):
        query_project = """
            SELECT "project_project".name, "project_project".id, "account_analytic_line".employee_id,sum("account_analytic_line".unit_amount) as total
        ...
        if len(selected_employee) > 0:
            query_project += """AND "account_analytic_line".employee_id IN (""" + ','.join(selected_employee) + """)"""
        if len(selected_project) > 0:
            query_project += """AND "account_analytic_line".project_id IN (""" + ','.join(selected_project) + """)"""
        if line_id :
            query_project += """AND "account_analytic_line".employee_id = """ + line_id + """ """

        query_project += """GROUP BY "project_project".id, "account_analytic_line".employee_id"""
        self.env.cr.execute(query_project)
        results = self.env.cr.dictfetchall()
        return results

    def get_all_employee(self,date_from, date_to, selected_employee, selected_project,line_id = None):
        sql_query = """
                    SELECT
        ...
        if line_id :
            sql_query += """AND "account_analytic_line".employee_id = """ + line_id + """ """
